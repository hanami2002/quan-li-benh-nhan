...
public class PaitnetDAO extends DBContextMySQL {
    ...
    public int countPaitnet() {
        String sql = "SELECT count(*)  FROM prj_qg.paitnet";
        try {
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public List<Paitnet> getListPaitnet(int pageIndex) {
        String sql = "SELECT *  FROM prj_qg.paitnet limit "+(10*(pageIndex-1))+",10";
        try {
            List<Paitnet> list = new ArrayList<>();
            ps = connection.prepareStatement(sql);
            rs = ps.executeQuery();
